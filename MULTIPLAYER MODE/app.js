import { draw as drawSnake, draw2 as drawSnake2, update as updateSnake, update2 as updateSnake2, getSnakeHead as snakeHead, getSnakeHead2 as snakeHead2, onSnake, expandSnake, onSnake2, expandSnake2, snakeBody, snakeBody2, onSnakeBody, onSnakeBody2 } from './functions.js'
...
function gameEngine() {
    ...
    const lives = document.querySelector(".lives1")
    // const life = document.querySelectorAll(".life")
    const lifeBox = document.querySelectorAll("#lifeBox1");
    const lifeBox2 = document.querySelectorAll("#lifeBox2");

    let initialLives = userInputLives;

    for (let i = 0; i < initialLives; i++) {
        lifeBox[i].classList.add("life");
        lifeBox2[i].classList.add("life");
    }
    function lifePowerUpFunction() {
        lifeBox[livesLeft].classList.add("life");
        livesLeft++;
    }
    function lifePowerUpFunction2() {
        lifeBox2[livesLeft2].classList.add("life");
        livesLeft2++;
    }
    ...
    function lifeDestructFunction2() {
        lifeBox2[livesLeft2 - 1].classList.remove("life");
        livesLeft2--;
    }
    ...
    let livesLeft2 = initialLives;
    function checkLivesLeft2() {
        let lifeLost = 0;
        let Over = outsideGrid(snakeHead2()) || snakeIntersection2();
        if (Over === true) {
            lifeBox2[livesLeft2 - 1].classList.remove("life")
            lifeLost = 1;
        } else {
        ...
        livesLeft2 = (livesLeft2 - lifeLost);
        return livesLeft2;
    }
    ...
    function snakeIntersection2() {
        if (over2 === 1) {
            return false;
        } else if (over2 === 0) {
            for (let i = 1; i <= snakeBody2.length - 1; i++) {
                if (snakeBody2[0].x === snakeBody2[i].x && snakeBody2[0].y === snakeBody2[i].y) {
                    return true;
                }
            }
        }
    ...
    function deathCheck2() {
        let livesLeft2 = checkLivesLeft2();
        if (livesLeft2 <= 0) {
            gameOver2 = true;
        }
        gameOverP2 = gameOver2 || getTimerText();
    }
    //lives left
    ...
    const currentScore = document.querySelector(".currentScore1");
    ...
    // const highScoreV = document.querySelector(".highScore1");
    ...
    function updateScore2() {
        currentScore2.innerText = `${score2}`;
        // currentScoreDisplay.innerText = `${score2}`;
        if (score > score2) {
            currentScoreDisplay.innerText = `${score}`;
            playerWon.innerText = "X WON"
            playerWon.classList.add("wonX");
        } else if (score2 > score) {
            currentScoreDisplay.innerText = `${score2}`;
            playerWon.innerText = "Y WON"
            playerWon.classList.add("wonY");
        } else {
            playerWon.innerText = "DRAW"
            playerWon.style.color = "grey"
            playerWon.style.fontSize = "5rem"
        }
    }
    ...
    function updatePowerUps() {
        ...
        for (let i = 0; i < bombObstacle.length; i++) {
            ...
            if (onSnakeBody2(bombObstacle[i])) {
                explosionSound.play();
                bombObstacle.splice(i, 1)
                lifeDestructFunction2();
            }
        ...
        if (onSnake2(portals[0])) {
            let inputDir2 = getInputDirection2();
            snakeBody2[0].x = portals[1].x + inputDir2.x;
            snakeBody2[0].y = portals[1].y + inputDir2.y;
        }
        ...
        if (onSnake2(portals[1])) {
            let inputDir2 = getInputDirection()
            snakeBody2[0].x = portals[0].x + inputDir2.x;
            snakeBody2[0].y = portals[0].y + inputDir2.y;
        }
    }
